...
"""
This def read the orginal porto taxi csv file and save all trips into
...
def porto2h5(csvfile::String)
    ...
    print("Processing $(size(df, 1)) trips...")
    ...
    h5open("porto.h5", "w") do f
        ...
        for trip in df[:POLYLINE]
            ...
            num % 100_000 == 0 && print("$num")
        end
        attrs(f)["num"] = num
        print("Saved $num trips.")
    end
end
...
"""
def distort(trip::Matrix{Float64}, rate::Float64; radius=50.0)
    noisetrip = copy(trip)
...
"""
def distort(trip::Matrix{Float64}, nsplit::Int)
...
"""
def downsampling(trip::Matrix{Float64}, rate::Float64)
    keep_idx = Int[1]
...
"""
def downsampling(trip::Matrix{Float64}, nsplit::Int)
    noisetrips = Matrix{Float64}[]
...
"""
def downsamplingDistort(trip::Matrix, nsplit::Int)
    noisetrips = Matrix{Float64}[]
...
"""
def lonlat2meters(lon::Float64, lat::Float64)
...
def meters2lonlat(x::Float64, y::Float64)
    semimajoraxis = 6378137.0

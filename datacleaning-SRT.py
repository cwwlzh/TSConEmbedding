...
#def gps2cell(region::SpatialRegion, lon::Float64, lat::Float64)
...
#end
#
#def cell2gps(region::SpatialRegion, cell::Int)
...
"""
def coord2cell(region::SpatialRegion, x::Float64, y::Float64)
...
"""
def cell2coord(region::SpatialRegion, cell::Int)
...
def inregion(region::SpatialRegion, lon::Float64, lat::Float64)
    lon >= region.minlon && lon < region.maxlon &&
    lat >= region.minlat && lat < region.maxlat
end

def inregion(region::SpatialRegion, trip::Matrix{Float64})
    for i = 1:size(trip, 2)
...
"""
def makeVocab!(region::SpatialRegion, trjfile::String)
    ...
    h5open(trjfile, "r") do f
        ...
        for i = 1:num
            ...
            i % 100_000 == 0 && print("Processed $i trips")
            #i >= 8_000 && break
        end
    end
    ...
    print("Cell count at max_num_hotcells:$(max_num_hotcells) is $(last(topcellcount[end]))")
    region.hotcell =
...
def knearestHotcells(region::SpatialRegion, cell::Int, k::Int)
...
def nearestHotcell(region::SpatialRegion, cell::Int)
    @assert region.built == true "Build index for region first"
...
"""
def saveKNearestVocabs(region::SpatialRegion)
    ...
    print("Saved cell distance into $file")
    nothing
end

"""
Return the vocab id for a cell in the region.
If the cell is not hot cell, the def will first search its nearest
hotcell and return the corresponding vocab id.
"""
def cell2vocab(region::SpatialRegion, cell::Int)
    @assert region.built == true "Build index for region first"
...
will be used.
"""
def gps2vocab(region::SpatialRegion, lon::Float64, lat::Float64)
    inregion(region, lon, lat) || return UNK
    cell2vocab(region, gps2cell(region, lon, lat))
end

def trip2seq(region::SpatialRegion, trip::Matrix{Float64})
    seq = Int[]
...
def seq2trip(region::SpatialRegion, seq::Vector{Int})
...
"""
def createTrainVal(region::SpatialRegion,
                        trjfile::String,
                        injectnoise::def,
                        ntrain::Int,
                        nval::Int;
    ...
    h5open(trjfile, "r") do f
        ...
        for i = 1:ntrain+nval
            ...
            i % 100_000 == 0 && print("Scaned $i trips...")
        end
...
def saveregion(region::SpatialRegion, paramfile::String)
    save(paramfile,
...
def loadregion!(region::SpatialRegion, paramfile::String)
